...
export async function uploadBufferToCdn(params: {
  ...
  const { buf, uploadParam, uploadUrl, filekey, cdnBaseUrl, aeskey } = params;
  const ciphertext = encryptAesEcb(buf, aeskey);
  const cdnUrl = uploadUrl ?? (
    uploadParam && filekey && cdnBaseUrl
      ? buildCdnUploadUrl({ cdnBaseUrl, uploadParam, filekey })
      : undefined
  );
  if (!cdnUrl) {
    throw new Error("CDN upload requires either uploadUrl or uploadParam + filekey + cdnBaseUrl");
  }
